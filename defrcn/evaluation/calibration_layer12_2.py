    ...
    def execute_calibration(self, inputs, dts):
        ...
        for i in range(ileft, iright):
            ...
            for class_label, prototype in self.prototypes.items():
                ...
                similarity = cosine_similarity(l2_normalize_(features[i - ileft].cpu().data.numpy()).reshape((1, -1)),
                                        prototype.cpu().data.numpy())[0][0]
                if similarity > max_similarity:
                    max_similarity = similarity
                    max_class = class_label
            
            max_similarity_tensor = torch.tensor(max_similarity, dtype=torch.float32)

# 将张量移动到与目标张量相同的设备上（CPU或CUDA）
            max_similarity_tensor = max_similarity_tensor.to(dts[0]['instances'].scores[i].device)

            dts[0]['instances'].scores[i] = max_similarity_tensor
            
            dts[0]['instances'].pred_classes[i] = max_class
