...
class PrototypicalCalibrationBlock:
    ...
    def build_prototypes(self):
        ...
        for label in features_dict: 
            ...
            # 使用 MiniBatchKMeans 聚类算法
            minibatch_kmeans = MiniBatchKMeans(n_clusters=num_prototypes_per_class, random_state=42)
            minibatch_kmeans.fit(features)
            
            # 获取每个簇的中心（原型）
            class_prototypes = minibatch_kmeans.cluster_centers_
            class_prototypes = torch.from_numpy(class_prototypes)
            
            # 将原型存储在字典中
            prototypes_dict[label] = class_prototypes  
    ...
    def execute_calibration(self, inputs, dts):
        ...
        for i in range(ileft, iright):   #遍历阈值内的框 i的作用
            ...
            for label, prototypes_list in self.prototypes.items():
                ...
                top_three_sim = sorted(cos_sim_list, reverse=True)[:3]
                avg_sim = sum(top_three_sim) / len(top_three_sim)
                if avg_sim > tmp_cos:
                    tmp_cos = avg_sim
                    predicate = label

            dts[0]['instances'].pred_classes[i]=predicate
            dts[0]['instances'].scores[i] =  tmp_cos 
            #原型和候选框的特征作度量 
