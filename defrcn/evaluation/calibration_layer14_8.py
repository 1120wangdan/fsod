...
# 遍历字典中的每个类别
    ...
    def execute_calibration(self, inputs, dts): 
        ...
        for i in range(ileft, iright):
            ...
            output_table = "Label: {}\nScore: {}".format(
                    " ".join(map(str, similarity_labels + [str(most_frequent_label), str(dts[0]['instances'].pred_classes[i])])), 
                        " ".join(map(str, similarity_scores + [str(average_similarity), str(dts[0]['instances'].scores[i])]))
                    )
